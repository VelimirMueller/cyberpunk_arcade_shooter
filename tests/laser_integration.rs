...
fn test_laser_phase_transitions_to_active() {
    assert_eq!(
        laser_phase_from_elapsed(LASER_CHARGE_DURATION),
        LaserPhase::Active
    );
    assert_eq!(
        laser_phase_from_elapsed(LASER_CHARGE_DURATION + 1.0),
        LaserPhase::Active
    );
}
...
fn test_shockwave_damages_boss() {
    ...
    app.world_mut()
        ...
        .unwrap()
        .current_hp = 80; // simulating 20 damage
...
fn test_shockwave_clears_projectiles() {
    ...
    let entities: Vec<Entity> = app
        .world_mut()
        .query_filtered::<Entity, With<BossProjectile>>()
        .iter(app.world())
        .collect();
